...
Gymnistics= df[df['Sport'] == 'Ice Hockey']
Medal_winners = Gymnistics[Gymnistics['Medal'] != 'NA']
medals_by_country = Medal_winners.groupby(['Name', 'Medal']).size().unstack(fill_value=0).head(5)
...
plt.bar(countries, bronze, label='Bronze', color='#cd7f32', width=0.2)
plt.bar(countries, silver, bottom=bronze, label='Silver', color='#c0c0c0', width=0.2)
plt.bar(countries, gold, bottom=bronze + silver, label='Gold', color='#ffd700', width=0.2)
